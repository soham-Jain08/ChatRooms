...
export default function ChatRoom() {
  ...
  const handleCreateRoom = async () => {
    ...
    if (!user) return;
    if (currentUser.role !== "Teacher") { alert("Only teachers can create rooms."); return; }
  ...
  const canManageRoom = (room) => {
    ...
    return currentUser.role === "Teacher" || room.createdBy === user.uid;
  };
  ...
  return (
    <div className="chat-container">
      <header>
        <h2>{selectedRoom ? selectedRoom.name : "Doubt Solver Chat"}</h2>
